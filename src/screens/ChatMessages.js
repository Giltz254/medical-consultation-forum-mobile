...
var socket;
const assets = "http://localhost:3000/";
//const assets = "http://10.10.247.80:3000/";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (message.images.length > 0 && message.images.length <= 10) {
        ...
        //const url = `http://10.10.247.80:3000/`; //local server for development
                          ...
                          <HStack flexWrap="wrap">
                            {message.files.map((image, i) => {
                              let path = hasFileExtension(image)
                                ? `https://elonkiptoo.s3.us-east-2.amazonaws.com/${image}`
                                : image;
                              return (
                                <Pressable
                                  key={i}
...
const MessageSent = ({ message, userData }) => {
  ...
  let imgPath = hasFileExtension(message.sender.photo)
    ? `https://elonkiptoo.s3.us-east-2.amazonaws.com/${message.sender.photo}`
